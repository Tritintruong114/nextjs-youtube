      ...
      >
        <div id="container" className="px-6 sm:px-24 w-full h-full">
          <div className="w-full relative mb-6 sm:mb-0">
            <Link href="/works">
              <button className="sm:absolute text-xl z-10 top-0 left-0 border-2 border-black rounded-full px-3 py-1 hover:bg-black bg-white hover:text-white duration-300 font-bebas">
                See Our Works
              </button>
            </Link>
          </div>
            ...
            {Expertises.map((expertise, index: number) => {
              return (
                <Link
                  key={index}
                  href="/works"
                  className="h-[450px] sm:h-full w-full border-2 border-black p-6 group sm:w-1/2 flex items-center justify-center flex-col"
                >
